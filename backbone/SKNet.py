...
class simple_SKConv(nn.Module):
    def __init__(self, channels, branches=2, groups=32, reduce=16, stride=1, len=32):
        super(simple_SKConv, self).__init__()
        len = max(channels // reduce, len)
        self.gap = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Sequential(
            nn.Conv2d(channels, len, kernel_size=1, stride=1, bias=False),
...
class fuse_sknet(nn.Module):
    def __init__(self, num_classes, num_block_lists=[3, 4, 6, 3]):
        super(fuse_sknet, self).__init__()
        self.basic_conv_Ori = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        ...
        self.stage_1_Ori = self._make_layer(64, 128, 256, nums_block=num_block_lists[0], stride=1)
        self.stage_2_Ori = self._make_layer(256, 256, 512, nums_block=num_block_lists[1], stride=2)
        self.stage_3_Ori = self._make_layer(512, 512, 1024, nums_block=num_block_lists[2], stride=2)

        self.basic_conv_Other = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        ...
        self.stage_1_Other = self._make_layer(64, 128, 256, nums_block=num_block_lists[0], stride=1)
        self.stage_2_Other = self._make_layer(256, 256, 512, nums_block=num_block_lists[1], stride=2)
        self.stage_3_Other = self._make_layer(512, 512, 1024, nums_block=num_block_lists[2], stride=2)

        for p in self.parameters():
            p.requires_grad = False

        self.skfuse = simple_SKConv(1024)
        self.stage_4 = self._make_layer(1024, 1024, 2048, nums_block=num_block_lists[3], stride=2)
    ...
    def forward(self, x_Ori, x_Other):
        x_Ori = self.basic_conv_Ori(x_Ori)
        x_Ori = self.stage_1_Ori(x_Ori)
        x_Ori = self.stage_2_Ori(x_Ori)
        x_Ori = self.stage_3_Ori(x_Ori)
        x_Other = self.basic_conv_Other(x_Other)
        x_Other = self.stage_1_Other(x_Other)
        x_Other = self.stage_2_Other(x_Other)
        x_Other = self.stage_3_Other(x_Other)
        x = self.skfuse([x_Ori, x_Other])
        # x = torch.cat([x_Ori, x_Gabor], 1)
        x = self.stage_4(x)
        x = self.gap(x)
        x = x.view(x.size(0), -1)
        x = self.classifier(x)
...
if __name__ == '__main__':
    logging.warning("just for debug, not use this.")
